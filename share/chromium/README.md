...
|-----------|--------------|--------------------|
| Extension JS/source while developing | nothing — run `tsk install chromium` | `0.2.0.N` (`N` auto-increments) |
| Releasing the project | `[workspace.package] version` only | exact `0.2.0` (packaged `/usr/share/tsk`) |
...
(that blocklists the id). To drop a high `0.2.0.N` back to a lower version,
