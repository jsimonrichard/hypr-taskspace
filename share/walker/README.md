...
- **Browsers** (chromium, firefox, …) → selected browser; Chromium-family shares the host profile by default (extensions and logins). Set `[browser].isolate_profile = true` in `~/.config/tsk/config.toml` for a per-task `--user-data-dir`. The first Chromium launch after restoring a task reopens that task's saved tabs. A new taskspace with no snapshot opens `.tsk/repo.toml` `[browser].default_tabs`, else the repo browse page, else one new tab.
