...
Pacman replaces `/usr/share/tsk` with remove-then-add. Hyprland sources those files directly and can auto-reload while they are missing. A PostTransaction hook runs `hyprctl reload`, restarts `tskd`, and (when already installed) copies `/usr/share/tsk/omarchy-plugin/` into `~/.config/omarchy/plugins/tsk.taskspace/` then `omarchy restart shell`. A locked session fails that hook with a warning; the package is already installed, but the running overlay is still the previous QML until you unlock and run `omarchy restart shell`. If you still see “could not find file” source errors, `hyprctl reload` is enough. The hook does **not** run `tsk install omarchy`, so cloned-menu patches (`SUPER+Space` Apps → `tsk launch`) are not applied on package install. You do not need `tsk install omarchy` for `tsk.taskspace` plugin code updates after the first install — only when changing user config integration (Lua bindings, `--tui` vs overlay, menu launch prefix). `tsk install omarchy` restarts the shell because the cloned menu is keepLoaded and ignores `rescanPlugins`.
...
2. Tag and push `v$pkgver` (`git tag v0.2.0 && git push origin v0.2.0`).
