...
The CRX version comes from the workspace package version in `Cargo.toml`. Bump that for a release — do not edit `share/chromium/extension/manifest.json`. From-source installs append a revision (`0.2.0.3`, …) so `tsk install chromium` updates Chromium without a version bump.
