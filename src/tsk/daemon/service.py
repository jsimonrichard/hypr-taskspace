...
from tsk.core.models import ContextMode, SessionState, Task, TaskStatus
from tsk.core.registry import Registry
from tsk.core.workspaces import default_taskspace_workspace_names
from tsk.daemon import workspace_nav
from tsk.daemon.waybar_export import write_waybar_file
from tsk.integrations import distrobox, git, hyprland
...
class TaskService:
    ...
    def _write_context_file(self, state: SessionState) -> None:
        try:
            runtime = xdg.tsk_runtime_dir()
            runtime.mkdir(parents=True, exist_ok=True)
            xdg.tsk_context_file().write_text(state.taskspace_label())
        except RuntimeError:
            pass
    ...
    def archive_task(self, task_id: str) -> None:
        from tsk.core import task_cleanup
    ...
    def open_terminal(
        ...
        *,
        ...
        if not tid:
            raise RuntimeError(
                "No task context. Run `tsk task switch <name>` or pass a task name."
            )
