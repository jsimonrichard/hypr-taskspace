...
app = typer.Typer(help="Install tsk integration with Hyprland and Waybar.")
...
def install_all(
    ...
    from tsk.install.hypr import install_hypr as do_install_hypr
    from tsk.install.hypr import plan_install as plan_hypr
    from tsk.install.waybar import install_waybar, plan_install as plan_waybar
...
def install_hypr(
    ...
    """Install tsk Hyprland keybinds and window rules."""
    from tsk.install.hypr import install_hypr as do_install
    from tsk.install.hypr import plan_install
...
def install_status_cmd() -> None:
    """Show install state."""
    from tsk.install.hypr import install_status as hypr_status
    from tsk.install.waybar import install_status as waybar_status
    ...
    w = waybar_status()
    if w["installed"] or w["tsk_modules_present"]:
        typer.echo("Waybar integration: installed")
        typer.echo(f"  config: {w['config_path']}")
        typer.echo(f"  tsk modules present: {w['tsk_modules_present']}")
    else:
...
def install_waybar_cmd(
    ...
    """Replace hyprland/workspaces with tsk task + desktop indicators."""
    from tsk.install.waybar import install_waybar, plan_install
