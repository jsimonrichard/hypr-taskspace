...
app = typer.Typer(
    name="tsk",
    help="Hypr Taskspace — task-centric Hyprland + Distrobox control plane.",
    no_args_is_help=True,
)

uninstall_app = typer.Typer(help="Remove tsk integrations.")


def _register_commands() -> None:
    """Import subcommand modules only when the CLI starts."""
    from tsk.cli import context, daemon, desktop, install, task, taskspace, waybar, workspace
...
def uninstall_hypr_root(
    ...
    from tsk.install.hypr import uninstall_hypr
...
def uninstall_waybar_root() -> None:
    ...
    from tsk.install.waybar import uninstall_waybar
...
@app.command()
def doctor() -> None:
    """Verify tsk installation and daemon health."""
    from tsk.daemon.server import is_daemon_running
    from tsk.install.hypr import doctor_checks as hypr_doctor
    ...
    if not is_daemon_running():
        typer.echo(
            ...
            "instead of per-poll DB writes; run `tsk daemon start` for best performance"
        )

    w = waybar_status()
    wb_ok = w["tsk_modules_present"]
    typer.echo(
        f"[{'ok' if wb_ok else 'FAIL'}] Waybar tsk workspace modules: {w['config_path']}"
    )
    if not wb_ok:
        ok = False
