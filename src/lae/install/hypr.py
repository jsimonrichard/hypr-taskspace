...
from lae.install.wrapper import (
    bin_dir,
    write_lae_wrapper,
...
def install_hypr(*, dry_run: bool = False, cfg: LaeConfig | None = None) -> InstallPlan:
    ...
    templates_installed.append({"generated": str(bin_dir(cfg) / "lae-task-tui")})
...
def install_status(cfg: LaeConfig | None = None) -> dict:
    ...
    tui_helper = cfg.install_hypr_share_dir / "bin" / "lae-task-tui"
    config_path = cfg.install_hypr_config_path
    has_source = False
    if config_path.exists():
...
def doctor_checks(cfg: LaeConfig | None = None) -> list[tuple[str, bool, str]]:
    ...
    checks.append(
        (
            ...
            str(cfg.install_hypr_share_dir / "bin" / "lae-task-tui"),
        )
    )
    checks.append(
        (
