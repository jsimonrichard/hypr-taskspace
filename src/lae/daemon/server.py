"""UNIX socket IPC server for lae daemon."""
...
from lae.integrations.hyprland_events import HyprlandEventListener, make_window_handler
...
class DaemonServer:
    def __init__(self, service: TaskService | None = None):
        ...
        self.socket_path = xdg.lae_daemon_socket()
...
def daemon_request(method: str, params: dict[str, Any] | None = None, timeout: float = 5.0) -> dict:
    path = xdg.lae_daemon_socket()
    if not path.exists():
        raise ConnectionError("lae daemon is not running")
