...
impl GrepDirPicker {
    ...
    pub fn register_at_cwd(existing: &[RegisteredRepo], cwd: &Path) -> Result<RegisteredRepo> {
        let root = detect_vcs_root(Some(cwd)).ok_or_else(|| {
            tsk_core::TskError::Other(format!(
    ...
    fn refresh(&mut self) -> Result<()> {
        ...
        for entry in std::fs::read_dir(&self.cwd).map_err(|source| tsk_core::TskError::Read {
            path: self.cwd.clone(),
            source,
        })? {
            let entry = entry.map_err(|source| tsk_core::TskError::Read {
                path: self.cwd.clone(),
                source,
            })?;
            let path = entry.path();
