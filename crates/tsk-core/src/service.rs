...
impl TaskService {
    ...
    pub fn create_task(
        ...
        let now = Utc::now();
        let browser_profile = crate::browser::default_browser_profile_dir(&self.config, &task_id);
        let mut task = Task {
            id: task_id.clone(),
            name: name.to_string(),
    ...
    pub fn open_url(&self, urls: &[&str], task_id: Option<&str>, host: bool) -> Result<()> {
        crate::browser::open_urls(urls, task_id, host)
    }
...
mod tests {
    ...
    #[test]
    fn tasks_for_menu_orders_most_recently_accessed_first() {
        let dir = tempdir().unwrap();
        let svc = test_service(dir.path());
        let older = svc
            .create_task(
        ...
        let newer = svc
            ...
            .unwrap();
        std::thread::sleep(std::time::Duration::from_millis(5));
        svc.switch_task(&older.id).unwrap();
        let older = svc
            .load_state()
            .unwrap()
            .tasks
        ...
        assert_eq!(items[0].last_active_at, None);
        ...
        assert_eq!(items[1].last_active_at, Some(older.last_active_at));
    }
