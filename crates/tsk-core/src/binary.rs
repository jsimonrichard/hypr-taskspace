...
/// Installed `tsk` binary, or `TSK` env when set to an executable path.
pub fn resolve_tsk_binary(cfg: &TskConfig) -> PathBuf {
    std::env::var("TSK")
        .ok()
        .map(PathBuf::from)
        .filter(|p| p.is_file())
        .unwrap_or_else(|| cfg.install_hypr_share_dir.join("bin/tsk"))
}
