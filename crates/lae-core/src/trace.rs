//! Opt-in latency tracing (`LAE_TRACE=1`).
//!
//! Maintained debugging surface:
//! - `LAE_TRACE=1` — enable in CLI or Waybar process
//! - `LAE_TRACE_FILE` — override log path (default: `$XDG_RUNTIME_DIR/lae/trace.log`)
//! - `lae debug trace show|analyze|clear|path`
//! - `lae debug trace workspace N` — switch workspace and print timeline
//! - `lae debug hyprland-socket` — socket2 availability (also in `lae doctor`)
...
pub fn enabled() -> bool {
    ...
    *ENV.get_or_init(|| {
        std::env::var("LAE_TRACE")
            .map(|v| matches!(v.to_ascii_lowercase().as_str(), "1" | "true" | "yes" | "on"))
            .unwrap_or(false)
    })
}

/// Turn tracing on for the remainder of this process (`lae debug trace workspace`).
pub fn enable_for_process() {
    FORCED.store(true, Ordering::Relaxed);
}

pub fn trace_path() -> Option<std::path::PathBuf> {
    if let Ok(path) = std::env::var("LAE_TRACE_FILE") {
...
/// Timeline for the most recent workspace switch (needs `LAE_TRACE=1` on Waybar).
pub fn analyze_recent_latency() -> LatencyReport {
    let lines = read_recent_lines(200).unwrap_or_default();
    if lines.is_empty() {
        return LatencyReport {
            note: Some(
                "No trace log. Set LAE_TRACE=1 for Waybar and run `lae debug trace workspace N`."
                    .into(),
            ),
            ..Default::default()
        };
    ...
    let Some(start) = anchor_idx else {
        return LatencyReport {
            lines: lines.clone(),
            note: Some(
                "No recent workspace switch in log. Run `lae debug trace workspace N` with \
                 LAE_TRACE=1 on Waybar."
                    .into(),
            ),
            ..Default::default()
        };
...
fn diagnostic_note(window: &[TraceLine]) -> Option<String> {
    ...
    if !has_waybar {
        return Some(
            "No waybar events — restart Waybar with LAE_TRACE=1.".into(),
        );
    }
