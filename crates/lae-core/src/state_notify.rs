...
pub fn state_events_socket_path() -> Result<PathBuf, crate::error::LaeError> {
    Ok(lae_runtime_dir()?.join("state-events.sock"))
}

fn state_rev_path() -> Result<PathBuf, crate::error::LaeError> {
    Ok(lae_runtime_dir()?.join("state.rev"))
}
