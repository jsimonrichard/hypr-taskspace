...
/// `~/lae-tasks/<task-id>/workspace/` — parent for all task checkouts.
...
mod tests {
    ...
    fn managed_checkout_detects_workspace_paths() {
        let base = PathBuf::from("/tmp/lae-tasks");
    ...
    fn linked_checkout_uses_repo_folder_name() {
        let home = PathBuf::from("/tmp/lae-tasks/t1");
        let source = PathBuf::from("/home/user/my-app");
        assert_eq!(
            linked_checkout_path(&home, &source),
            PathBuf::from("/tmp/lae-tasks/t1/workspace/my-app")
        );
    }
}
