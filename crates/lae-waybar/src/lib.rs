...
fn install_bar_button_styles(root: &GtkBox) {
    ...
    if let Err(err) = provider.load_from_data(BAR_BUTTON_CSS.as_bytes()) {
        eprintln!("lae-waybar: failed to load bar button CSS: {err}");
        return;
    }
    root.style_context()
...
impl Module for LaeBar {
    ...
    fn init(info: &InitInfo, _config: Config) -> Self {
        ...
        task_button.set_widget_name("lae-task-btn");
        ...
        if state_listener.is_none() {
            eprintln!("lae-waybar: state-events socket unavailable — using rev polling");
        }
        trace_event(
            "waybar",
            "init",
